...
- OpenCode v2.0.3 Promise `AgentDomain` extends the generated `AgentApi`. Its `agent.list()` operation returns `AgentListOutput`, whose pinned shape is `{ location, data: AgentInfo[] }`; the generated Promise client returns that envelope directly. The v2 adapter therefore models the host-facing envelope explicitly and unwraps only `data` at one generation boundary before capability logic enumerates Agents. Array-direct fakes are not a valid model of the pinned v2 host contract.
